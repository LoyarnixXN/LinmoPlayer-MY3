...
当前可以验证导航、Material 3 视觉层级、本地文件导入、播放/暂停和插件空状态。移动端需要在真机或模拟器上选择音频文件；Windows 可直接选择本机音频文件。

首次打开会显示三步新手引导：导入音乐、点击歌曲播放、了解插件扩展边界。引导完成或跳过后会保存在本地；Windows 可清除应用本地存储后重新查看，Android 可清除应用数据后重新查看。

当前仍未实现第三方音源、插件动态执行和歌单网络同步；这些都没有被偷偷实现。本地导入的文件目前只保存在当前运行会话中，下一阶段再接入本地数据库和持久化索引。
