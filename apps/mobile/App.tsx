...
type LocalPlugin = { id: string; name: string; version: string; capabilities: string[]; description: string; category: Exclude<PluginCategory, 'all' | 'installed'>; enabled: boolean };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.appShell}>
        ...
        <MiniPlayer song={currentSong} songs={songs} isPlaying={isPlaying} onToggle={togglePlaying} onPlaySong={playSong} />
            ...
            return (
              ...
              >
                <View style={[styles.navIcon, selected && styles.navIconSelected]}>
                  <AppIcon name={tab.icon} size={22} color={selected ? colors.onSecondaryContainer : colors.onSurfaceVariant} />
                </View>
...
function LibraryScreen({ songs, onImport, onPlaySong, currentSong }: { songs: readonly LocalSong[]; onImport: () => void; onPlaySong: (song: LocalSong) => void; currentSong: LocalSong | null }) {
  ...
  const categories: readonly [LibraryCategory, string][] = [['all', '全部'], ['songs', '歌曲'], ['albums', '专辑'], ['artists', '歌手']];
  ...
  const renderSong = (song: LocalSong) => <SongRow key={song.key} song={song} index={songs.indexOf(song)} active={song.key === currentSong?.key} onPress={() => onPlaySong(song)} />;
  ...
  const formatCount = new Set(songs.map((song) => song.mediaUri.split('.').pop()?.toUpperCase()).filter(Boolean)).size;
  return (
    <>
      ...
      <View style={styles.libraryStats}>
        ...
        <LibraryStat label="专辑" value={albumCount} caption={albumCount ? '已识别' : '等待元数据'} />
        <LibraryStat label="歌手" value={artistCount} caption={artistCount ? '已识别' : '等待元数据'} />
        <LibraryStat label="格式" value={formatCount || '—'} caption="文件格式" />
      </View>
      <View style={styles.filterRow}>
        {categories.map(([id, label]) => <Pressable key={id} onPress={() => setCategory(id)} accessibilityRole="tab" accessibilityState={{ selected: category === id }}><Chip label={label} selected={category === id} /></Pressable>)}
        <Pressable onPress={onImport}><Chip label="导入本地音乐" /></Pressable>
      </View>
      {songs.length === 0 ? <EmptyLibrary onImport={onImport} /> : category === 'albums' || category === 'artists' ? (
        <View style={styles.libraryGroups}>
          {[...groupedSongs.entries()].map(([label, group]) => <View key={label} style={styles.libraryGroup}>
            <View style={styles.libraryGroupHeader}><Text style={styles.libraryGroupTitle}>{label}</Text><Text style={styles.libraryGroupCount}>{group.length} 首</Text></View>
...
function LibraryStat({ label, value, caption }: { label: string; value: number | string; caption: string }) {
  return <View style={styles.libraryStat}><Text style={styles.libraryStatLabel}>{label}</Text><Text style={styles.libraryStatValue}>{value}</Text><Text style={styles.libraryStatCaption}>{caption}</Text></View>;
}

function EmptyLibrary({ onImport }: { onImport: () => void }) {
  return <View style={styles.libraryEmpty}><View style={styles.libraryEmptyIcon}><AppIcon name="musical-notes-outline" size={27} color={colors.primary} /></View><View style={styles.libraryEmptyCopy}><Text style={styles.libraryEmptyTitle}>暂无音乐</Text><Text style={styles.libraryEmptyBody}>当前音乐库为空，导入本地音频文件后将在此显示。</Text><View style={styles.libraryEmptyMeta}><Text style={styles.libraryEmptyMetaLabel}>支持格式</Text><Text style={styles.libraryEmptyMetaValue}>MP3 · M4A · WAV</Text></View></View><Pressable style={[styles.outlineButton, styles.libraryEmptyButton]} accessibilityRole="button" onPress={onImport}><Text style={styles.outlineButtonText}>导入本地音乐</Text></Pressable></View>;
}
...
function PluginsScreen() {
  ...
  const categories: readonly [PluginCategory, string][] = [['all', '全部'], ['installed', '已安装'], ['sources', '音源'], ['tools', '工具']];
  const detailOpacity = useRef(new Animated.Value(0)).current;
  useEffect(() => { void AsyncStorage.getItem(PLUGINS_STORAGE_KEY).then((value) => { if (value) setPlugins(JSON.parse(value)); }).catch(() => undefined); }, []);
  useEffect(() => { Animated.timing(detailOpacity, { toValue: showDetails ? 1 : 0, duration: 240, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start(); }, [detailOpacity, showDetails]);
  if (showDetails) return <Animated.View style={[styles.settingsDetail, { opacity: detailOpacity }]}><Pressable onPress={() => setShowDetails(false)} accessibilityRole="button"><View style={styles.settingsBackRow}><AppIcon name="chevron-back" size={17} color={colors.primary} /><Text style={styles.settingsBack}>返回插件中心</Text></View></Pressable><Text style={styles.sectionTitle}>插件说明</Text><Text style={styles.settingsDescription}>插件通过统一契约接入搜索、播放、歌词、歌单和账户等能力，并由宿主统一管理。</Text><Text style={styles.settingsNote}>导入插件清单后，可查看版本、能力范围并控制启用状态。插件只能调用清单中声明的宿主能力。</Text></Animated.View>;
  const filteredPlugins = plugins.filter((plugin) => category === 'all' || category === 'installed' || plugin.category === category);
  const importPlugin = async () => {
    const result = await getDocumentAsync({ type: 'application/json', copyToCacheDirectory: true });
    if (result.canceled || !result.assets?.[0]?.uri) return;
    try {
      const input = JSON.parse(await (await fetch(result.assets[0].uri)).text());
      const manifest = input.manifest ?? input;
      const rawCapabilities: unknown[] = Array.isArray(manifest.capabilities) ? manifest.capabilities : [];
      const capabilities = [...new Set(rawCapabilities.filter((capability): capability is string => typeof capability === 'string'))];
      if (!/^[a-z0-9][a-z0-9._-]{1,63}$/.test(manifest.id) || !manifest.name || !manifest.version || String(manifest.hostApiVersion).split('.')[0] !== '1') throw new Error('插件清单格式不符合当前宿主版本。');
      const plugin: LocalPlugin = { id: manifest.id, name: String(manifest.name), version: String(manifest.version), capabilities, description: typeof manifest.description === 'string' ? manifest.description : '未提供插件说明。', category: capabilities.some((capability) => ['search', 'playback', 'lyrics', 'recommendations'].includes(capability)) ? 'sources' : 'tools', enabled: false };
    ...
    } catch (error) { Alert.alert('插件清单无效', error instanceof Error ? error.message : '无法读取插件清单。'); }
  };
  const togglePlugin = async (id: string) => { const next = plugins.map((plugin) => plugin.id === id ? { ...plugin, enabled: !plugin.enabled } : plugin); setPlugins(next); await AsyncStorage.setItem(PLUGINS_STORAGE_KEY, JSON.stringify(next)); };
  const removePlugin = async (id: string) => { const next = plugins.filter((plugin) => plugin.id !== id); setPlugins(next); await AsyncStorage.setItem(PLUGINS_STORAGE_KEY, JSON.stringify(next)); };
  return (
    <>
      <View style={styles.libraryHeader}><Text style={styles.libraryCount}>{plugins.length} 个插件</Text><Pressable onPress={importPlugin}><Chip label="导入插件" /></Pressable></View>
      <View style={styles.filterRow}>{categories.map(([id, label]) => <Pressable key={id} onPress={() => setCategory(id)} accessibilityRole="tab" accessibilityState={{ selected: category === id }}><Chip label={label} selected={category === id} /></Pressable>)}</View>
      {filteredPlugins.length ? <View style={styles.pluginList}>{filteredPlugins.map((plugin) => <View key={plugin.id} style={styles.pluginCard}><View style={styles.pluginIcon}><AppIcon name="extension-puzzle-outline" size={24} color={colors.primary} /></View><View style={styles.pluginCopy}><Text style={styles.pluginTitle}>{plugin.name}</Text><Text style={styles.pluginDescription}>{plugin.description}</Text><Text style={styles.pluginMeta}>v{plugin.version} · {plugin.enabled ? '已启用' : '已停用'}</Text></View><View style={styles.pluginActions}><Pressable style={[styles.smallOutlineButton, plugin.enabled && styles.pluginEnabledButton]} onPress={() => void togglePlugin(plugin.id)}><Text style={styles.outlineButtonText}>{plugin.enabled ? '停用' : '启用'}</Text></Pressable><Pressable onPress={() => void removePlugin(plugin.id)}><Text style={styles.pluginRemove}>卸载</Text></Pressable></View></View>)}</View> : <View style={styles.libraryEmpty}><View style={styles.libraryEmptyIcon}><AppIcon name="extension-puzzle-outline" size={27} color={colors.primary} /></View><View style={styles.libraryEmptyCopy}><Text style={styles.libraryEmptyTitle}>{category === 'all' ? '暂无已导入插件' : '此分类暂无插件'}</Text><Text style={styles.libraryEmptyBody}>{category === 'all' ? '导入插件清单后，可在此管理版本、能力与启用状态。' : '当前没有符合此分类的插件。'}</Text><Pressable style={[styles.outlineButton, styles.libraryEmptyButton]} accessibilityRole="button" onPress={importPlugin}><Text style={styles.outlineButtonText}>导入插件</Text></Pressable></View><Pressable onPress={() => setShowDetails(true)}><Text style={styles.pluginDocsLink}>查看插件说明</Text></Pressable></View>}
    </>
  );
}
...
function SettingsScreen({ section, songCount, backgroundPlayback, onSelect, onBack, onToggleBackgroundPlayback, onClearLibrary, onResetOnboarding }: { section: SettingsSection | null; songCount: number; backgroundPlayback: boolean; onSelect: (section: SettingsSection) => void; onBack: () => void; onToggleBackgroundPlayback: () => void; onClearLibrary: () => void; onResetOnboarding: () => void }) {
  ...
  useEffect(() => {
    ...
    Animated.parallel([
      Animated.timing(listOpacity, { toValue: 1, duration: 280, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      Animated.timing(listOffset, { toValue: 0, duration: 320, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
    ]).start();
  ...
  if (section === 'privacy') {
    return <SettingsDetail title="隐私与安全" description="Linmo Player 的代码、插件契约与数据处理边界公开可审查。" onBack={onBack}>
      <Text style={styles.settingsNote}>本项目采用开源架构，音乐文件、插件配置与播放器偏好均由本地应用管理。你可以自行审查源代码、插件清单和权限声明。</Text>
    ...
    <Animated.View style={{ opacity: listOpacity, transform: [{ translateX: listOffset }] }}>
      <View style={styles.settingsCard}>
        {items.map(([id, title, body], index) => (
          <Pressable key={id} style={({ pressed }) => [styles.settingRow, index < items.length - 1 && styles.settingRowBorder, pressed && styles.buttonPressed]} onPress={() => onSelect(id)} accessibilityRole="button">
...
function PlaybackToggle({ enabled }: { enabled: boolean }) {
  ...
  useEffect(() => {
    Animated.timing(progress, { toValue: enabled ? 1 : 0, duration: 220, easing: Easing.inOut(Easing.cubic), useNativeDriver: false }).start();
  }, [enabled, progress]);
  const trackColor = progress.interpolate({ inputRange: [0, 1], outputRange: [colors.outlineVariant, colors.primary] });
  const thumbOffset = progress.interpolate({ inputRange: [0, 1], outputRange: [0, 18] });
  return <Animated.View style={[styles.toggle, { backgroundColor: trackColor }]}><Animated.View style={[styles.toggleThumb, { transform: [{ translateX: thumbOffset }] }]} /></Animated.View>;
}
...
function SettingsDetail({ title, description, onBack, children }: { title: string; description: string; onBack: () => void; children: ReactNode }) {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.timing(offset, { toValue: 0, duration: 300, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
    ]).start();
  }, [offset, opacity]);
  return <Animated.View style={[styles.settingsDetail, { opacity, transform: [{ translateX: offset }] }]}><Pressable onPress={onBack} accessibilityRole="button"><View style={styles.settingsBackRow}><AppIcon name="chevron-back" size={17} color={colors.primary} /><Text style={styles.settingsBack}>返回应用设置</Text></View></Pressable><Text style={styles.sectionTitle}>{title}</Text><Text style={styles.settingsDescription}>{description}</Text><View style={styles.settingsDetailCard}>{children}</View></Animated.View>;
}
...
function SongRow({ song, index, active, onPress }: { song: UnifiedSong; index: number; active: boolean; onPress: () => void }) {
  return (
    <Pressable style={({ pressed }) => [styles.songRow, pressed && styles.songRowPressed]} onPress={onPress} accessibilityRole="button" accessibilityLabel={`${song.title}，${song.artist}`}>
      ...
      <AppIcon name={active ? 'musical-notes' : 'ellipsis-horizontal'} size={19} color={active ? colors.primary : colors.onSurfaceVariant} />
    </Pressable>
  );
}
...
function CoverTile({ index, size }: { index: number; size: number }) {
  ...
  const glyphs: readonly IconName[] = ['musical-note-outline', 'disc-outline', 'radio-outline', 'sparkles-outline'];
  const backgroundColor = coverColors[index % coverColors.length]!;
  const glyph = glyphs[index % glyphs.length]!;
  return <View style={[styles.coverTile, { width: size, height: size, backgroundColor }]}><AppIcon name={glyph} size={Math.round(size * 0.45)} color={colors.primary} /></View>;
}
...
function MiniPlayer({ song, songs, isPlaying, onToggle, onPlaySong }: { song: LocalSong | null; songs: readonly LocalSong[]; isPlaying: boolean; onToggle: () => void; onPlaySong: (song: LocalSong) => void }) {
  ...
  return <View style={styles.miniPlayer}>{song ? <><CoverTile index={0} size={44} /><View style={styles.miniInfo}><Text style={styles.miniTitle} numberOfLines={1}>{song.title}</Text><Text style={styles.miniArtist} numberOfLines={1}>{song.artist}</Text></View></> : <Text style={styles.miniEmpty}>还没有正在播放的歌曲</Text>}<Pressable disabled={!previousSong} onPress={() => previousSong && onPlaySong(previousSong)} accessibilityRole="button" accessibilityLabel="上一首" style={styles.miniControl}><AppIcon name="chevron-back" size={21} color={previousSong ? colors.onSurfaceVariant : colors.outlineVariant} /></Pressable><Pressable disabled={!song} onPress={onToggle} accessibilityRole="button" accessibilityLabel={isPlaying ? '暂停' : '播放'} style={styles.miniButton}><AppIcon name={isPlaying ? 'pause' : 'play'} size={15} color={song ? colors.onPrimary : colors.outlineVariant} /></Pressable><Pressable disabled={!nextSong} onPress={() => nextSong && onPlaySong(nextSong)} accessibilityRole="button" accessibilityLabel="下一首" style={styles.miniControl}><AppIcon name="chevron-forward" size={21} color={nextSong ? colors.onSurfaceVariant : colors.outlineVariant} /></Pressable></View>;
}

const onboardingSlides = [
  { icon: 'musical-notes-outline', eyebrow: '1 / 3', title: '导入本地音乐', body: '请选择设备中的 MP3、M4A 或 WAV 音频文件。', action: '导入音乐' },
  { icon: 'play', eyebrow: '2 / 3', title: '播放与控制', body: '选择任意曲目即可播放，可通过底部播放器进行暂停与继续。', action: '下一步' },
  { icon: 'extension-puzzle-outline', eyebrow: '3 / 3', title: '插件扩展', body: '后续可通过插件接入外部音源，当前版本仅支持本地音乐。', action: '开始使用' },
] as const;
...
const styles = StyleSheet.create({
  ...
  heroCard: { minHeight: 220, borderRadius: radii.card, backgroundColor: colors.primaryContainer, padding: spacing.lg, overflow: 'hidden', flexDirection: 'row', justifyContent: 'space-between', marginBottom: spacing.xl + spacing.sm },
  ...
  primaryButton: { backgroundColor: colors.primary, borderRadius: radii.control, minHeight: 48, paddingHorizontal: spacing.md, marginTop: spacing.lg, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, width: 138 },
  ...
  libraryHeader: { flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: spacing.lg },
  libraryCount: { color: colors.onSurfaceVariant, fontSize: 13 },
  libraryStats: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, marginBottom: spacing.lg },
  libraryStat: { flexGrow: 1, flexBasis: '46%', minHeight: 84, padding: spacing.md, borderRadius: radii.control, backgroundColor: colors.surfaceContainer },
  ...
  libraryGroupHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: spacing.xs, paddingHorizontal: spacing.xs },
  ...
  libraryEmpty: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.lg, backgroundColor: colors.surfaceContainer, borderRadius: radii.card },
  libraryEmptyIcon: { width: 58, height: 58, borderRadius: 18, backgroundColor: colors.secondaryContainer, alignItems: 'center', justifyContent: 'center' },
  libraryEmptyCopy: { flex: 1 },
  libraryEmptyTitle: { color: colors.onSurface, fontSize: 18, fontWeight: '700' },
  libraryEmptyBody: { color: colors.onSurfaceVariant, fontSize: 13, lineHeight: 20, marginTop: spacing.xs },
  libraryEmptyMeta: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginTop: spacing.sm },
  ...
  pluginCard: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, padding: spacing.md, borderRadius: radii.card, backgroundColor: colors.surfaceContainer },
  pluginIcon: { width: 48, height: 48, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primaryContainer },
  pluginCopy: { flex: 1 },
  ...
  pluginDocsLink: { color: colors.primary, fontSize: 12, fontWeight: '700', padding: spacing.sm, marginLeft: spacing.sm },
  ...
  settingsCard: { backgroundColor: colors.surfaceContainer, borderRadius: radii.card, paddingHorizontal: spacing.md, marginTop: spacing.lg },
  settingRow: { minHeight: 84, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  ...
  settingsDescription: { color: colors.onSurfaceVariant, fontSize: 13, lineHeight: 20, marginTop: spacing.sm, marginBottom: spacing.xl },
  ...
  miniPlayer: { marginHorizontal: spacing.md, marginBottom: spacing.md, minHeight: 72, padding: spacing.md, paddingRight: spacing.lg, borderRadius: radii.control, backgroundColor: colors.surfaceContainerHigh, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
