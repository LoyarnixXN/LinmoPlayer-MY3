import { createMusicSourcePlugin, validatePluginPackageManifest } from './vendor/linmo-core.js';
...
let pluginCatalog = [];
try {
  pluginCatalog = JSON.parse(localStorage.getItem('linmo-plugins') || '[]');
} catch {
  pluginCatalog = [];
}
const preferences = {
...
try {
  Object.assign(preferences, JSON.parse(localStorage.getItem('linmo-preferences') || '{}'));
} catch {}
try {
  const savedLibrary = JSON.parse(localStorage.getItem('linmo-library') || '[]');
  if (Array.isArray(savedLibrary))
    songs.push(...savedLibrary.filter((song) => typeof song?.localPath === 'string'));
} catch {}
...
function savePlugins() {
  try {
    localStorage.setItem('linmo-plugins', JSON.stringify(pluginCatalog));
  } catch {}
}
...
function normalizePluginManifest(input) {
  ...
  if (
    ...
    throw new Error('插件 ID 需使用 2—64 位小写字母、数字、点、短横线或下划线。');
  if (typeof manifest.name !== 'string' || !manifest.name.trim())
    throw new Error('插件名称不能为空。');
  if (typeof manifest.version !== 'string' || !manifest.version.trim())
    throw new Error('插件版本不能为空。');
  if (String(manifest.hostApiVersion).split('.')[0] !== '1')
    throw new Error('插件需要不兼容的宿主 API 版本。');
  if (capabilities.some((capability) => !supportedCapabilities.includes(capability)))
...
function importPluginFiles(fileList) {
  [...fileList].forEach((file) => {
    file
      .arrayBuffer()
      .then((bytes) => window.linmoDesktop.plugins.readPackage(bytes))
      .then((plugin) => {
        try {
          const normalizedPlugin = normalizePluginManifest(plugin);
          const existing = pluginCatalog.findIndex((item) => item.id === normalizedPlugin.id);
          if (existing >= 0)
            pluginCatalog[existing] = {
              ...pluginCatalog[existing],
              ...normalizedPlugin,
              installed: true,
            };
          else pluginCatalog.push(normalizedPlugin);
          savePlugins();
          renderProfileMenu();
          renderPlugins();
        } catch (error) {
          window.alert(error instanceof Error ? error.message : '插件 ZIP 无法读取。');
        }
      })
      .catch((error) =>
        window.alert(error instanceof Error ? error.message : '插件 ZIP 无法读取。'),
...
function removePlugin(pluginId) {
  pluginCatalog = pluginCatalog.filter((plugin) => plugin.id !== pluginId);
  savePlugins();
  renderProfileMenu();
  renderPlugins();
}
...
const iconPaths = {
  ...
  shuffle:
    '<path d="M4 7h3c4 0 6 10 10 10h3M17 5l3 2-3 2M4 17h3c1.3 0 2.3-1.2 3.1-2.7M17 15l3 2-3 2"/>',
  repeat: '<path d="m17 4 3 3-3 3M20 7H8a4 4 0 0 0-4 4v1M7 20l-3-3 3-3M4 17h12a4 4 0 0 0 4-4v-1"/>',
  repeatOne:
    '<path d="m17 4 3 3-3 3M20 7H8a4 4 0 0 0-4 4v1M7 20l-3-3 3-3M4 17h12a4 4 0 0 0 4-4v-1"/><path d="M12 9v6M10.5 10l1.5-1v6"/>',
...
function icon(name, extra = '') {
  return `<svg class="ui-icon ${extra}" viewBox="0 0 24 24" aria-hidden="true" focusable="false">${iconPaths[name] || iconPaths.music}</svg>`;
}
function cover(iconName = 'music', extra = '') {
  return `<div class="cover ${extra}">${icon(iconName)}</div>`;
}
function savePreferences() {
  try {
    localStorage.setItem('linmo-preferences', JSON.stringify(preferences));
  } catch {}
}
function clearLibrary() {
  ...
  songs.length = 0;
  try {
    localStorage.removeItem('linmo-library');
  } catch {}
  currentSong = null;
  isPlaying = false;
...
function resetOnboarding() {
  try {
    localStorage.removeItem('linmo-onboarding-complete');
  } catch {}
  onboardingStep = 0;
  renderOnboarding();
}
function updateScrollIndicator() {
  ...
  const thumbHeight = Math.max(
    28,
    Math.min(44, (trackHeight * scrollContainer.clientHeight) / scrollContainer.scrollHeight),
  );
...
async function importFiles() {
  if (window.linmoDesktop?.library?.pickAudio) {
    const paths = await window.linmoDesktop.library.pickAudio();
    addAudioPaths(paths);
  } else document.querySelector('#audio-files').click();
}
function onboardingCompleted() {
  try {
    return localStorage.getItem('linmo-onboarding-complete') === '1';
  } catch {
    return false;
  }
}
function finishOnboarding() {
  try {
    localStorage.setItem('linmo-onboarding-complete', '1');
  } catch {}
...
function renderOnboarding() {
  ...
  document.querySelector('#onboarding-primary').addEventListener('click', () => {
    if (onboardingStep === 0) importFiles();
    if (onboardingStep === onboardingSlides.length - 1) finishOnboarding();
    else {
      onboardingStep += 1;
      renderOnboarding();
    }
...
function playSong(song) {
  ...
  if (currentSong !== song) {
    ...
    if (song.file) {
      ...
      audio.src = objectUrl;
    } else audio.src = `file:///${String(song.localPath || '').replaceAll('\\', '/')}`;
    audio.currentTime = 0;
    currentSong = song;
...
function updateMiniProgress() {
  ...
  if (progress)
    progress.value = String(
      hasDuration ? Math.max(0, Math.min(100, (audio.currentTime / audio.duration) * 100)) : 0,
    );
...
function playAdjacent(direction) {
  ...
  if (preferences.playbackMode === 'repeat-all')
    targetIndex = (targetIndex + songs.length) % songs.length;
...
function seekAudio(value) {
  if (Number.isFinite(audio.duration))
    audio.currentTime = Math.max(
      0,
      Math.min(audio.duration, (Number(value) / 100) * audio.duration),
    );
}
...
function renderHome() {
  ...
  document
    .querySelectorAll('[data-song-index]')
    .forEach((row) =>
      row.addEventListener('click', () => playSong(songs[Number(row.dataset.songIndex)])),
    );
}
function renderLibrary() {
  ...
  const filterMarkup = categories
    .map(
      ([id, label]) =>
        `<button type="button" class="library-filter ${libraryCategory === id ? 'active' : ''}" data-library-category="${id}">${label}</button>`,
    )
    .join('');
  ...
  const extensions = [
    ...new Set(
      songs.map((song) => song.file?.name?.split('.').pop()?.toUpperCase()).filter(Boolean),
    ),
  ];
  const statsMarkup = [
    ['曲目', songs.length, '已导入音频'],
    ['专辑', albumCount, albumCount ? '已识别' : '等待元数据'],
    ['歌手', artistCount, artistCount ? '已识别' : '等待元数据'],
    ['格式', extensions.length ? extensions.join(' / ') : '—', '文件格式'],
  ]
    .map(
      ([label, value, caption]) =>
        `<div class="library-stat"><span>${label}</span><strong>${value}</strong><small>${caption}</small></div>`,
    )
    .join('');
  ...
  if (songs.length) {
    if (libraryCategory === 'albums' || libraryCategory === 'artists') {
      ...
      songs.forEach((song) => {
        const label =
          libraryCategory === 'albums' ? song.album || '未知专辑' : song.artist || '未知歌手';
        if (!groups.has(label)) groups.set(label, []);
        groups.get(label).push(song);
      });
      content = [...groups.entries()]
        .map(
          ([label, group]) =>
            `<section class="library-group"><div class="library-group-heading"><h3>${label}</h3><span>${group.length} 首</span></div><div class="song-list">${group
              .map((song) => {
                const index = songs.indexOf(song);
                return `<div class="song-row" data-library-song-index="${index}">${cover(song.glyph, 'small')}<div class="song-copy"><div class="song-title">${song.title}</div><div class="song-artist">${song.artist}${song.album ? ` · ${song.album}` : ''}</div></div></div>`;
              })
              .join('')}</div></section>`,
        )
        .join('');
  ...
  document.querySelectorAll('[data-library-category]').forEach((button) =>
    button.addEventListener('click', () => {
      libraryCategory = button.dataset.libraryCategory;
      renderLibrary();
    }),
  );
  document
    .querySelectorAll('[data-library-song-index]')
    .forEach((row) =>
      row.addEventListener('click', () => playSong(songs[Number(row.dataset.librarySongIndex)])),
    );
}
function renderPlugins() {
  const root = document.querySelector('#page-plugins');
  if (pluginDetailsOpen) {
    root.innerHTML = `<div class="settings-detail"><button class="text-button settings-back" id="plugin-back">${icon('arrowLeft', 'back-icon')}返回插件中心</button><div class="eyebrow">PLUGIN API</div><h2>插件说明</h2><p class="settings-description">插件通过统一契约接入搜索、播放、歌词、歌单和账户等能力，并由宿主统一管理。</p><div class="settings-note">导入插件 ZIP 后，可在插件中心查看版本、能力范围并控制启用状态。宿主会先校验 ZIP 清单、路径和入口文件。</div></div>`;
    document.querySelector('#plugin-back').addEventListener('click', () => {
      pluginDetailsOpen = false;
      renderPlugins();
    });
    return;
  }
  const filterMarkup = pluginCategories
    .map(
      ([id, label]) =>
        `<button type="button" class="library-filter ${pluginCategory === id ? 'active' : ''}" data-plugin-category="${id}">${label}</button>`,
    )
    .join('');
  const filteredPlugins = pluginCatalog.filter(
    (plugin) =>
      pluginCategory === 'all' ||
      plugin.category === pluginCategory ||
      (pluginCategory === 'installed' && plugin.installed),
  );
  const content = filteredPlugins.length
    ? `<div class="plugin-grid">${filteredPlugins.map((plugin) => `<article class="plugin-card">${cover('plugins', 'small')}<div class="plugin-copy"><strong>${escapeHtml(plugin.name)}</strong><p>${escapeHtml(plugin.description)}</p><small>v${escapeHtml(plugin.version)} · ${plugin.enabled ? '已启用' : '已停用'}</small><div class="plugin-capabilities">${plugin.capabilities.map((capability) => `<span>${escapeHtml(capability)}</span>`).join('')}</div></div><div class="plugin-card-actions"><button class="plugin-toggle ${plugin.enabled ? 'active' : ''}" type="button" data-plugin-toggle="${escapeHtml(plugin.id)}">${plugin.enabled ? '停用' : '启用'}</button><button class="text-button plugin-remove" type="button" data-plugin-remove="${escapeHtml(plugin.id)}">卸载</button></div></article>`).join('')}</div>`
    : `<div class="empty compact plugin-empty">${cover('plugins', 'empty-icon')}<h2>${pluginCategory === 'all' ? '暂无已导入插件' : '此分类暂无插件'}</h2><p>${pluginCategory === 'all' ? '导入插件清单后，可在此管理插件版本、能力与启用状态。' : '当前没有符合此分类的插件。'}</p><button class="outline-button" id="plugin-import-empty">导入插件</button><button class="text-button" id="plugin-docs">查看插件说明</button></div>`;
  root.innerHTML = `<div class="section-heading library-actions"><span class="muted">${pluginCatalog.length} 个插件</span><div class="plugin-actions"><button class="text-button" id="plugin-import">导入插件 ZIP</button><button class="text-button" id="plugin-docs-top">插件说明</button></div></div><div class="library-filters">${filterMarkup}</div>${content}`;
  document
    .querySelector('#plugin-import')
    ?.addEventListener('click', () => document.querySelector('#plugin-files').click());
  document
    .querySelector('#plugin-import-empty')
    ?.addEventListener('click', () => document.querySelector('#plugin-files').click());
  document.querySelector('#plugin-docs-top')?.addEventListener('click', () => {
    pluginDetailsOpen = true;
    renderPlugins();
  });
  document.querySelector('#plugin-docs')?.addEventListener('click', () => {
    pluginDetailsOpen = true;
    renderPlugins();
  });
  document
    .querySelectorAll('[data-plugin-toggle]')
    .forEach((button) =>
      button.addEventListener('click', () => togglePlugin(button.dataset.pluginToggle)),
    );
  document
    .querySelectorAll('[data-plugin-remove]')
    .forEach((button) =>
      button.addEventListener('click', () => removePlugin(button.dataset.pluginRemove)),
    );
  document.querySelectorAll('[data-plugin-category]').forEach((button) =>
    button.addEventListener('click', () => {
      pluginCategory = button.dataset.pluginCategory;
      renderPlugins();
    }),
  );
}
function renderSettings() {
  ...
  if (!settingsSection) {
    ...
    root.querySelectorAll('[data-setting]').forEach((button) =>
      button.addEventListener('click', () => {
        settingsSection = button.dataset.setting;
        renderSettings();
      }),
    );
    return;
  ...
  root.querySelectorAll('[data-preference]').forEach((button) =>
    button.addEventListener('click', () => {
      const key = button.dataset.preference;
      preferences[key] = !preferences[key];
      savePreferences();
      button.classList.toggle('active', preferences[key]);
  ...
  document.querySelector('#playback-mode')?.addEventListener('change', (event) => {
    preferences.playbackMode = event.target.value;
    savePreferences();
    renderMiniPlayer();
    renderSettings();
  });
...
function renderMiniPlayer() {
  ...
  const songMarkup = currentSong
    ? `${cover(currentSong.glyph, 'small')}<div class="mini-copy"><strong>${currentSong.title}</strong><small>${currentSong.artist}${currentSong.album ? ` · ${currentSong.album}` : ''}</small></div>`
    : `${cover('music', 'small mini-placeholder')}<div class="mini-copy"><strong>未选择歌曲</strong><small>导入音乐后可开始播放</small></div>`;
  ...
  document
    .querySelector('[data-player-action="mode"]')
    ?.addEventListener('click', cyclePlaybackMode);
  document
    .querySelector('[data-player-action="previous"]')
    ?.addEventListener('click', playPrevious);
  document
    .querySelector('[data-player-action="next"]')
    ?.addEventListener('click', () => playAdjacent(1));
  document.querySelector('[data-player-action="mute"]')?.addEventListener('click', toggleMute);
  document
    .querySelector('#mini-progress')
    ?.addEventListener('input', (event) => seekAudio(event.target.value));
  document
...
document.querySelector('#audio-files').addEventListener('change', (event) => {
  if (event.target.files?.length) addFiles(event.target.files);
  event.target.value = '';
});
document.querySelector('#plugin-files')?.addEventListener('change', (event) => {
  if (event.target.files?.length) importPluginFiles(event.target.files);
  event.target.value = '';
});
document.querySelectorAll('.nav-item').forEach((button) =>
  button.addEventListener('click', () => {
    ...
    document
      ...
      .forEach((item) => item.classList.toggle('active', item.id === `page-${page}`));
    document.querySelector('#page-subtitle').textContent = pageSubtitles[page];
    requestAnimationFrame(updateScrollIndicator);
  }),
);
...
document
  ...
  ?.addEventListener('click', () => windowApi?.toggleMaximize());
...
document.querySelector('#profile-trigger')?.addEventListener('click', (event) => {
  ...
  const menu = document.querySelector('#profile-menu');
  const isOpen = Boolean(menu && !menu.hidden && !menu.classList.contains('is-closing'));
  setProfileMenuOpen(!isOpen);
});
document.addEventListener('click', (event) => {
  const wrap = document.querySelector('.profile-wrap');
  if (wrap && !wrap.contains(event.target)) setProfileMenuOpen(false);
});
document.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'ArrowLeft' && currentSong) {
    event.preventDefault();
    seekAudio(((audio.currentTime - 5) / Math.max(audio.duration, 1)) * 100);
  }
  if (event.key === 'ArrowRight' && currentSong) {
    event.preventDefault();
    seekAudio(((audio.currentTime + 5) / Math.max(audio.duration, 1)) * 100);
  }
...
window.addEventListener('blur', () => {
  if (!preferences.backgroundPlayback && isPlaying) audio.pause();
});
